...
__forceinline Vec3::Vec3() {}

// W set to 1.0f, representing a point.
__forceinline Vec3::Vec3(float xIn, float yIn, float zIn)
{
    _data = _mm_setr_ps(xIn, yIn, zIn, 1.0f);
}
__forceinline Vec3::Vec3(float xIn, float yIn, float zIn, float wIn)
{
    _data = _mm_setr_ps(xIn, yIn, zIn, wIn);
}
__forceinline Vec3::Vec3(const __m128 &data)
{
    _data = data;
}
...
__forceinline float Vec3::X() const { return _data.m128_f32[0]; }
__forceinline float Vec3::Y() const { return _data.m128_f32[1]; }
__forceinline float Vec3::Z() const { return _data.m128_f32[2]; }
__forceinline float Vec3::W() const { return _data.m128_f32[3]; }

__forceinline void Vec3::Set(float xIn, float yIn, float zIn, float wIn)
{
    _data = _mm_setr_ps(xIn, yIn, zIn, wIn);
}
#pragma endregion
...
__forceinline void Vec3::operator=(const Vec3 &rhs)
{
    _data = rhs._data;
}
__forceinline Vec3 Vec3::operator+(const Vec3 &rhs)
{
...
__forceinline Vec3 Vec3::operator/(float divisor)
{
    if (divisor == 0)
    {
...
__forceinline void Vec3::operator*=(float scalar)
{
    __m128 temp = _mm_setr_ps(scalar, scalar, scalar, 1.0f);
    _data = _mm_mul_ps(_data, temp);
}
__forceinline void Vec3::operator/=(float divisor)
{
    if (divisor == 0)
    {
        throw(std::exception("Vec3: divisor is zero!"));
...
__forceinline void Vec3::Add(const Vec3 &toAdd)
{
    _data = _mm_add_ps(_data, toAdd._data);
}
__forceinline void Vec3::Scale(float scalar)
{
    (*this) *= scalar;
}

// Squared length of the vector.
__forceinline float Vec3::SqLen()
{
...
// Set W to 1.
__forceinline void Vec3::ToPoint()
{
    _data = _mm_insert_ps(_data, _mm_set_ps1(1.0f), 0b11110000);
}

// Set W to 0.
__forceinline void Vec3::ToDir()
{
...
__forceinline Vec3 Vec3::Normalized()
{
    __m128 temp = _mm_dp_ps(_data, _data, 0b01110111);
    temp = _mm_rsqrt_ps(temp);
    temp = _mm_insert_ps(temp, _mm_set_ps1(0.0f), 0b11110000); // set w of temp to 0.0f
...
__forceinline Vec3 Vec3::Mul(const Matrix4x4 &matrix)
{
...
__forceinline Vec3 Vec3::operator*(const Matrix4x4 &matrix)
{
    return Mul(matrix);
}
